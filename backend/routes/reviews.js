...
const { getApplicationReviews, getUserReviews, getReview, updateReview } = require("../services/reviews");
...
router.get('/:id', isAuthenticated, (req, res, next) => {
  getReview(req.params.id).then((review) => {
    res.status(200).json({
      review: review 
    });
  }).catch((err) => {
...
router.put('/:id', [
  ...
  updateReview(review_update, req.user).then((review) => {
    res.status(200).json({
      review: review 
    });
  }).catch((err) => {
