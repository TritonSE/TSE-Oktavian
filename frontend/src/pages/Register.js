...
export default function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (submission.roles.length === 0) {
      setState({...state, form_disabled: false, snack: {message: 'You must select at least one application type.', open: true}});
      return;
    }
    try {
      ...
      if (response.ok) {
        ...
        setState({...state, form_disabled: false, snack: {message: 'Invalid secret value.', open: true}});
      }
      else if (response.status === 409) {
        setState({...state, form_disabled: false, snack: {message: 'An account with that email already exists.', open: true}});
      }
      else {
    ...
    >
      <Grid item md={6} xs={12}>
        ...
        <form className={classes.form} onSubmit={handleSubmit}>
          ...
          <FormControl>
            <FormLabel component="legend">What types of applications are you responsible for?</FormLabel>
            <FormGroup>
              <FormControlLabel
                control={<Checkbox onChange={handleChecked('role_project_manager')} name="role-project-manager"/>}
                label="Project Managers"
              />
              <FormControlLabel
                control={<Checkbox onChange={handleChecked('role_developer')} name="role-developer"/>}
                label="Developers"
              />
              <FormControlLabel
                control={<Checkbox onChange={handleChecked('role_designer')} name="role-designer"/>}
                label="Designers"
              />
            </FormGroup>
          </FormControl>
          <FormControl>
            <FormLabel component="legend">Are you cleared to make final decisions?</FormLabel>
            <FormControlLabel
              control={<Checkbox onChange={handleChecked('role_final')} name="role-final"/>}
              label="Yes"
            />
            <FormHelperText>Do not select this unless you are the president. Your registration will be flagged.</FormHelperText>
          </FormControl>
          <TextField label='Secret' variant='outlined' type='password' onChange={handleChange('secret')}/>
          <FormHelperText className={classes.lightSpacing}>This secret is required for registration and is only distributed internally.</FormHelperText>
